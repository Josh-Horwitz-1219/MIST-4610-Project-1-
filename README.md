...
On the other side of the model, the Teams table is attached to the Matches table. Since one team can play in many matches, and only one team from Velocity Vortex Athletics can participate in a match at a time, there is a one-to-many relationship between teams and matches, with one team being able to play in many different matches. Next, the Tournaments table has information about when and where each tournament took place. The Matches and Tournaments table is linked by a one-to-many relationship since one tournament can consist of many matches while matches can only belong to one tournament. After the Tournaments table, we have the Sponsors table. Since one sponsor can sponsor many tournaments and a tournament can have many sponsors, there is a many-to-many relationship between Sponsors and Tournaments. It is for this reason that there is an associative entity between the Tournaments and Sponsors tables entitled Tournament Sponsors. This table contains information on which specific sponsors helped to fund specific  tournaments. 
...
### Query 1  
This query lists the player's full name, the name of the team they are on, and their team’s number of players for teams that have 5 or more wins.
...
### Query 8
This query lists the session ID, facility ID, facility name, and player name(players that are 16 and older) for all the training sessions that took place in January and February, where players' age is greater than the average age of players 
...
Associating sponsors with tournaments held at Eagle Crest Arena and Liberty Fields Arena facilitates relationship strengthening and engagement efforts. By identifying sponsors and their representatives concerning these venues, tournament organizers can initiate and maintain communication channels more effectively. This allows for ongoing collaboration, feedback exchange, and partnership development, fostering long-term relationships that benefit both parties.
